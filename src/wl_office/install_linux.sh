...
#  RPMCMD="--freshen"
  ...
  if [ "$ADD" = "no" ]; then
    #Hacked for populating the private rpm database with the dependencies needed
    rpm --initdb --dbpath $RPM_DB_PATH
    rpm --rebuilddb --dbpath $RPM_DB_PATH
    for i in ` ls /var/lib/rpm/ ` ; do 
        file /var/lib/rpm/$i | grep "Berkeley DB" -q && cp /var/lib/rpm/$i $RPM_DB_PATH/$i ; done
    for i in `ls $PACKAGE_PATH/*.rpm` ; do rpm -e ` basename $i | sed "s/.rpm//g" `  --nodeps --dbpath $RPM_DB_PATH &> /dev/null ; done
  fi
...
UserInstallation=\$BRAND_BASE_DIR/../UserInstallation rpm $RPMCMD --force --ignoresize -vh $RELOCATIONS --dbpath $RPM_DB_PATH $RPMLIST  
...
then
  ...
  rpm --erase -v  --dbpath $RPM_DB_PATH `rpm --query --queryformat "%{NAME} " --package $RPMLIST --dbpath $RPM_DB_PATH`
  echo
  echo "Now re-installing new packages .."
  echo
  rpm --install --force  --ignoresize -vh $RELOCATIONS --dbpath $RPM_DB_PATH $RPMLIST 
  echo
fi
